...
def build_character_user_prompt(
    *,
    ...
    return (
        ...
        "- `alias` is OPTIONAL in meaning: if the text does not explicitly provide an alias/title/epithet/role for this character, set `alias` to an empty string \"\".\n"
        "- Only use a NON-empty `alias` when it is EXPLICITLY mentioned in the story text (e.g., \"the king\", \"old woman\"). Do NOT invent aliases based on common sense or general knowledge. The alias must be directly quoted or clearly referenced in the provided text.\n"
        "- A character may have multiple aliases/titles in the text; if so, include ALL explicitly-mentioned aliases in `alias`, separated by `;` (e.g., \"the king; His Majesty\").\n"
